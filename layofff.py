...
@app.route("/")
def welcome():
    return render_template("thankssunil.html")
...
def linechart():
    conn = sqlite3.connect('layoff.sqlite')
    conn.row_factory = sqlite3.Row
    temp = conn.execute('SELECT DATEPART("month", Date) AS Date, SUM(Laid_Off_Count) AS Count FROM layoff GROUP BY DATEPART("month", Date)').fetchall()
    conn.close()
    result = [{k: item[k] for k in item.keys()} for item in temp]
    return jsonify(result)
